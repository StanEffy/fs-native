  ...
  describe("RepositoryListContainer", () => {
    it("renders repository information correctly", () => {
      ...
      expect(firstRepositoryItem).toHaveTextContent(
        `${getNumber(first.stargazersCount)}`
      );
      expect(secondRepositoryItem).toHaveTextContent(
        `${getNumber(second.stargazersCount)}`
      );

      expect(firstRepositoryItem).toHaveTextContent(
        `${getNumber(first.forksCount)}`
      );
      expect(secondRepositoryItem).toHaveTextContent(
        `${getNumber(second.forksCount)}`
      );

      expect(firstRepositoryItem).toHaveTextContent(
        `${getNumber(first.ratingAverage)}`
      );
      expect(secondRepositoryItem).toHaveTextContent(
        `${getNumber(second.ratingAverage)}`
      );

      expect(firstRepositoryItem).toHaveTextContent(
        `${getNumber(first.reviewCount)}`
      );
      expect(secondRepositoryItem).toHaveTextContent(
        `${getNumber(second.reviewCount)}`
      );
    });
  });
});
